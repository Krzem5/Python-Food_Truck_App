...
if (os.path.exists("build")==False):
	...
	if (ge==True):
		...
		if (subprocess.run(["git","init"]).returncode!=0):
			os.chdir(cwd)
			quit()
		if (subprocess.run(["git","config","user.email",f"\"{EMAIL}\""]).returncode!=0):
			os.chdir(cwd)
			quit()
		if (subprocess.run(["git","config","user.name",f"\"{USER_NAME}\""]).returncode!=0):
			os.chdir(cwd)
			quit()
		if (subprocess.run(["heroku.cmd","git:remote","-a",APP_NAME]).returncode!=0):
			os.chdir(cwd)
			quit()
		os.chdir(cwd)
